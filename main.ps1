...
.DESCRIPTION
    Executes every preparation, OpenCore generation, and disk preparation stage in
    isolated Windows PowerShell 5.1 child processes. A non-zero exit code stops the
    pipeline immediately. The selected macOS target is propagated through
...
function Invoke-PipelineStep {
    ...
    $previousOffset=$env:DEVINTOSH_GLOBAL_STEP_OFFSET;$previousTotal=$env:DEVINTOSH_GLOBAL_STEP_TOTAL;$previousStageTotal=$env:DEVINTOSH_GLOBAL_STAGE_TOTAL;$previousStopOnWarning=$env:DEVINTOSH_STOP_ON_WARNING
    try{$env:DEVINTOSH_GLOBAL_STEP_OFFSET=[string]$GlobalStepOffset;$env:DEVINTOSH_GLOBAL_STEP_TOTAL=[string]$GlobalStepTotal;$env:DEVINTOSH_GLOBAL_STAGE_TOTAL=[string]$StageStepTotal;if($FailOnWarning){$env:DEVINTOSH_STOP_ON_WARNING='1'}else{Remove-Item Env:DEVINTOSH_STOP_ON_WARNING -ErrorAction SilentlyContinue};$stageStartedAt=Get-Date;& powershell.exe @arguments;$code=[int]$LASTEXITCODE}
    finally{if($null-eq$previousOffset){Remove-Item Env:DEVINTOSH_GLOBAL_STEP_OFFSET -ErrorAction SilentlyContinue}else{$env:DEVINTOSH_GLOBAL_STEP_OFFSET=$previousOffset};if($null-eq$previousTotal){Remove-Item Env:DEVINTOSH_GLOBAL_STEP_TOTAL -ErrorAction SilentlyContinue}else{$env:DEVINTOSH_GLOBAL_STEP_TOTAL=$previousTotal};if($null-eq$previousStageTotal){Remove-Item Env:DEVINTOSH_GLOBAL_STAGE_TOTAL -ErrorAction SilentlyContinue}else{$env:DEVINTOSH_GLOBAL_STAGE_TOTAL=$previousStageTotal};if($null-eq$previousStopOnWarning){Remove-Item Env:DEVINTOSH_STOP_ON_WARNING -ErrorAction SilentlyContinue}else{$env:DEVINTOSH_STOP_ON_WARNING=$previousStopOnWarning}}
...
try {
    ...
    Write-Host 'Exit code 0 permits continuation, including advisory WARN entries.';Write-Host 'No stage after a failure or blocking warning is executed.';Write-Host 'Boot-artifact verification runs before destructive disk preparation.';Write-Host '============================================================'
