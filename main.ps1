...
.DESCRIPTION
    ...
    after a failure. The final stage intentionally invokes prepare-boot-disk.ps1
    without a disk number so disk selection remains interactive and hardware-agnostic.
...
try {
    if (-not (Test-Path -LiteralPath (Join-Path $repoRoot '.git') -PathType Container)) {
        throw 'main.ps1 must be executed from a clean cloned Devintosh repository.'
    }
    ...
    foreach ($scriptName in $requiredScripts) {
        if ($scriptName -eq 'prepare-boot-disk.ps1') {
            Write-Host ''
            Write-Host '[MAIN] Reaching final disk setup. Disk selection remains interactive.'
        }
