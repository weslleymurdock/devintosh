...
try {
    ...
    if($mac.exists -and -not $mac.valid){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw "Native macOS validation report is malformed: $($mac.path): $($mac.error)"}
    ...
    if($blocked.Count -gt 0){$EXIT_CODE=$script:EXIT_TARGET_NOT_FOUND;throw "Readiness is blocked: $($blocked[0])"}
    ...
    if($unknown.Count -gt 0){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw "Readiness is blocked: $($unknown[0].path) has unknown state '$($unknown[0].status)'."}
    ...
    $v=@($loaded|Where-Object{$_.key -eq 'validation'})[0];$vs=Get-Status $v;$exit=Get-PropertyValue $v.data 'validatorExitCode';$ver=[string](Get-PropertyValue $v.data 'openCoreVersion');if($vs -ne 'Valid' -or $null -eq $exit -or [int]$exit -ne 0){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw 'Final ocvalidate report is not valid with exit code 0.'}
    $versionPath=Join-Path $script:RepoRoot 'config\versions\sequoia.json';if(-not(Test-Path $versionPath)){$EXIT_CODE=$script:EXIT_TARGET_NOT_FOUND;throw 'Pinned macOS/OpenCore version configuration is missing.'}else{$vc=Get-Content $versionPath -Raw -Encoding UTF8|ConvertFrom-Json;$expected=[string](Get-PropertyValue(Get-PropertyValue $vc 'opencore') 'version');if(-not[string]::IsNullOrWhiteSpace($expected)-and$ver-ne$expected){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw "Validation used OpenCore '$ver' but pinned configuration requires '$expected'."}}
    Write-DevintoshStepLog $step "Final OpenCore validation: $vs; version: $ver." 'PASS'

    $step++;Write-DevintoshProgress $step $totalSteps 'Computing conservative readiness decision'
    $status='Ready';$reasons=[System.Collections.Generic.List[string]]::new();if($needsProfile.Count -gt 0){$status='NeedsProfile';foreach($s in $needsProfile){[void]$reasons.Add("$($s.key) remains unresolved or requires a matching capability profile.")}}elseif($needsValidation.Count -gt 0){$status='NeedsValidation';foreach($s in $needsValidation){[void]$reasons.Add("$($s.key) remains validation-required.")}}else{[void]$reasons.Add('All required build-time and native macOS runtime capabilities are resolved, and the generated config.plist passed the pinned ocvalidate.')}
