...
.DESCRIPTION
    ...
    common.ps1. The persistent download/cache root is outside the repository and
    defaults to D:\DevintoshRecovery through DEVINTOSH_RECOVERY_DOWNLOAD_ROOT.

    Temporary Apple downloads are created under that external root, never under
    build\ or any other repository directory. A verified payload is committed to
    the persistent cache and can therefore survive a clean clone/retry.

    The repository receives only the canonical build/recovery copy consumed by
    later stages. That copy is disposable and may be recreated from the external
    cache without contacting Apple again.
...
$downloadRoot = $script:Devintosh.Recovery.DownloadRoot
...
$tempRoot = Join-Path $downloadRoot ("recovery-download-" + [Guid]::NewGuid().ToString('N'))
...
function Invoke-MacRecoveryChunkValidation {
    ...
    & $PythonCommand @arguments 2>&1 | ForEach-Object {
        $line = "$_"
        if ($line.Trim().Length -gt 0) { Write-DevintoshLog 'INFO' "chunk-verify: $line" }
    }
...
function Test-RecoverySet {
    ...
    try {
        ...
        Write-DevintoshLog 'WARN' "Recovery chunk verification failed: $($_.Exception.Message)"
        return $false
    }
}
...
function Write-RecoveryManifest {
    ...
    $manifest = [ordered]@{
        ...
        culture = 'en-US'
        targetId = $script:Devintosh.Version.Id
        macOSFamily = $script:Devintosh.Version.MacOSFamily
        macOSMajorVersion = $script:Devintosh.Version.MacOSMajorVersion
        boardId = $boardId
        mlbMode = 'generic'
        osType = $osType
        source = 'Apple Internet Recovery via OpenCore macrecovery'
        opencoreRepository = $script:Devintosh.OpenCore.Repository
        opencoreVersion = $script:Devintosh.OpenCore.Version
        opencoreCommit = $script:Devintosh.OpenCore.PinnedCommit
...
try {
    ...
    $step++
    Write-DevintoshProgress $step $totalSteps 'Loading the centralized macOS Recovery contract'
    Write-DevintoshStepLog $step "$($script:Devintosh.Version.MacOSFamily) Recovery selected; pinned chunklist SHA-256: $expectedChunkSha." 'PASS'

    $step++
    Write-DevintoshProgress $step $totalSteps 'Preparing the external persistent Recovery storage'
    foreach ($directory in @($downloadRoot,$cacheRoot,$cachePath,$recoveryRoot,$toolRoot)) {
        if (-not (Test-Path -LiteralPath $directory)) { New-Item -ItemType Directory -Path $directory -Force | Out-Null }
    }
    Write-DevintoshLog 'INFO' "DEVINTOSH_RECOVERY_DOWNLOAD_ROOT=$env:DEVINTOSH_RECOVERY_DOWNLOAD_ROOT"
    Write-DevintoshLog 'INFO' "DEVINTOSH_RECOVERY_CACHE=$env:DEVINTOSH_RECOVERY_CACHE"
    Write-DevintoshLog 'INFO' "Recovery temporary download path=$tempRoot"
    Write-DevintoshLog 'INFO' "Recovery persistent cache path=$cachePath"
    ...
    $baseUrl = "https://raw.githubusercontent.com/{0}/{1}/{2}" -f $script:Devintosh.OpenCore.Repository,$script:Devintosh.OpenCore.PinnedCommit,$script:Devintosh.OpenCore.ToolPath
    if (-not (Test-Path -LiteralPath $pythonPath -PathType Leaf)) { Invoke-DownloadTextFile -Uri "$baseUrl/macrecovery.py" -Destination $pythonPath }
    if (-not (Test-Path -LiteralPath $batPath -PathType Leaf)) { Invoke-DownloadTextFile -Uri "$baseUrl/macrecovery.bat" -Destination $batPath }
    ...
    if (Test-Path -LiteralPath $cacheManifestPath -PathType Leaf) {
        try {
            $cached = Get-Content -LiteralPath $cacheManifestPath -Raw -Encoding UTF8 | ConvertFrom-Json
            $metadata = ([string]$cached.targetId -eq [string]$script:Devintosh.Version.Id) -and ([string]$cached.boardId -eq $boardId) -and ([string]$cached.osType -eq $osType) -and ([string]$cached.pinnedChunklistSha256).ToLowerInvariant() -eq $expectedChunkSha
            if ($metadata -and (Test-RecoverySet -Root $cachePath -VerifierPath $pythonPath -PythonCommand $pythonCommand -UsesLauncher $pythonUsesLauncher)) {
                ...
                Write-DevintoshStepLog $step 'Recovery restored from persistent external storage.' 'PASS'
                $step++
    ...
    $step++
    Write-DevintoshProgress $step $totalSteps 'Downloading and verifying Apple Recovery outside the repository'
    ...
    Write-DevintoshStepLog $step "Apple Recovery downloaded and macrecovery chunk-verified. Pinned chunklist SHA-256: $expectedChunkSha." 'PASS'

    $step++
    Write-DevintoshProgress $step $totalSteps 'Committing verified Recovery to external persistent storage'
    ...
    try { $ok = Invoke-DevintoshRollback; if (-not $ok) { $EXIT_CODE = $script:EXIT_ROLLBACK_FAILURE } }
