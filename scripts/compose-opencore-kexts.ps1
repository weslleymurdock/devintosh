...
.DESCRIPTION
    ...
    bundle metadata is inspected from Contents/Info.plist and the executable is verified.

    Kexts marked requiresValidation remain in Kernel/Add but are disabled. This prevents
    an unvalidated hardware profile from becoming active while preserving the resolved
    artifact for later validation.
#>
[CmdletBinding()]
param([switch]$Force)
...
function Get-PlistDictionary {
    ...
    foreach($name in $Path){
        ...
        if($null -eq $key){
            ...
            $dict=$current.OwnerDocument.CreateElement('dict');[void]$current.AppendChild($dict);$current=$dict;continue
        }
        $value=$key.NextSibling;while($null -ne $value -and $value.NodeType -ne 'Element'){$value=$value.NextSibling}
...
function New-KernelAddEntry {
    ...
    foreach($pair in @(
        ...
        $key=$Document.CreateElement('key');$key.InnerText=[string]$pair[0];[void]$dict.AppendChild($key)
...
function Resolve-KextBundleMetadata {
    ...
    $info=New-Object System.Xml.XmlDocument;$info.PreserveWhitespace=$true
    ...
    $executableNode=$executableKey.NextSibling;while($null -ne $executableNode -and $executableNode.NodeType -ne 'Element'){$executableNode=$executableNode.NextSibling}
...
function Set-KernelAdd {
    ...
    if($null -eq $existingKey){$addKey=$kernel.OwnerDocument.CreateElement('key');$addKey.InnerText='Add';[void]$kernel.AppendChild($addKey)}else{$addKey=$existingKey}
    $old=$addKey.NextSibling;while($null -ne $old -and $old.NodeType -ne 'Element'){$old=$old.NextSibling};if($null -ne $old){[void]$kernel.RemoveChild($old)}
...
try {
    ...
    foreach($entry in $entries){$item=Resolve-KextBundleMetadata -StageRoot $stageRoot -Entry $entry;$key=$item.bundlePath.ToLowerInvariant();if($bundleNames.ContainsKey($key)){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw "Duplicate kext BundlePath '$($item.bundlePath)' detected."};$bundleNames[$key]=$true;[void]$metadata.Add($item)}
    ...
    foreach($entry in $entries){$item=@($metadata|Where-Object{$_.id -eq [string](Get-PropertyValue $entry 'id')});if($item.Count -ne 1){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw "Unable to correlate staged kext '$([string](Get-PropertyValue $entry 'id'))' with its metadata."};[void]$ordered.Add($item[0])}
    $seen=@{}
    foreach($item in $ordered){foreach($dependency in @($item.dependencies)){if(-not $seen.ContainsKey([string]$dependency)){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw "Kext '$($item.id)' appears before its declared dependency '$dependency'."}};$seen[$item.id]=$true}
    $disabled=@($ordered|Where-Object{$_.requiresValidation}|ForEach-Object{$_.id});if($disabled.Count -gt 0){Write-DevintoshLog 'WARN' "Validation-required kexts will be present but disabled: $($disabled -join ', ')."}
    ...
    $root=[System.Xml.XmlElement]$xml.DocumentElement.SelectSingleNode('/plist/dict');if($null -eq $root){$EXIT_CODE=$script:EXIT_ASSET_INTEGRITY_FAILURE;throw 'Generated config.plist has an invalid plist root.'}
    ...
    $backupPath=Join-Path $script:BackupRoot ("config-kexts-{0}.plist" -f $backupStamp);Copy-Item -LiteralPath $configPath -Destination $backupPath -Force
    ...
    $report=[ordered]@{schemaVersion=1;generatedAtUtc=(Get-Date).ToUniversalTime().ToString('o',[Globalization.CultureInfo]::InvariantCulture);sourceAssets='build/opencore/kext-assets.json';status=$(if($disabled.Count -gt 0){'NeedsValidation'}else{'Applied'});entries=@($ordered|ForEach-Object{[ordered]@{id=$_.id;version=$_.version;bundlePath=$_.bundlePath;executablePath=$_.executablePath;plistPath=$_.plistPath;enabled=$_.enabled;requiresValidation=$_.requiresValidation;dependencies=@($_.dependencies);stagedPath=$_.stagedPath;payloadSha256=$_.payloadSha256}});disabledValidationRequiredKexts=@($disabled);generatedArtifact='build/efi/EFI/OC/config.plist'}
    $report|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $reportPath -Encoding UTF8
    Complete-DevintoshTransaction;Write-DevintoshStepLog $step 'Kext composition report written.' 'PASS';Complete-DevintoshProgress 'OpenCore Kernel Add composition complete';exit $script:EXIT_SUCCESS
}
catch{
    Write-DevintoshStepLog $step "OpenCore kext composition failed: $($_.Exception.Message)" 'FAIL';Write-DevintoshLog 'ERROR' $_.Exception.ToString();$rollbackOk=Invoke-DevintoshRollback;if(-not $rollbackOk){$EXIT_CODE=$script:EXIT_ROLLBACK_FAILURE};exit $EXIT_CODE
}
