...
.DESCRIPTION
    ...
    PlatformInfo/Generic is explicitly enabled and its update switches are enabled so
    the ephemeral bootstrap SMBIOS identity is actually consumed by OpenCore.
.PARAMETER Force
...
try{
    ...
    $step++;Write-DevintoshProgress $step $totalSteps 'Preparing first-boot PlatformInfo and SecureBoot policy';$document=New-Object System.Xml.XmlDocument;$document.PreserveWhitespace=$true;$document.Load($configPath);$root=$document.SelectSingleNode('/plist/dict');if($null -eq $root){$EXIT_CODE=$script:EXIT_ASSET_INTEGRITY_FAILURE;throw 'Invalid OpenCore plist root.'};$generic=Get-PlistDict ([System.Xml.XmlElement]$root) @('PlatformInfo','Generic');$platform=Get-PlistDict ([System.Xml.XmlElement]$root) @('PlatformInfo');$security=Get-PlistDict ([System.Xml.XmlElement]$root) @('Misc','Security');$backup="$configPath.firstboot-backup";Copy-Item -LiteralPath $configPath -Destination $backup -Force;Add-DevintoshRollbackAction -Name 'Restore OpenCore config before first-boot policy change' -Action {Copy-Item -LiteralPath $backup -Destination $configPath -Force};Set-PlistValue $generic 'Enabled' 'boolean' 'true';Set-PlistValue $platform 'Automatic' 'boolean' 'true';Set-PlistValue $platform 'UpdateDataHub' 'boolean' 'true';Set-PlistValue $platform 'UpdateNVRAM' 'boolean' 'true';Set-PlistValue $platform 'UpdateSMBIOS' 'boolean' 'true';Set-PlistValue $platform 'UpdateSMBIOSMode' 'string' 'Create';Set-PlistValue $platform 'UpdateSMBIOS' 'boolean' 'true';Set-PlistValue $security 'SecureBootModel' 'string' 'Disabled';Save-PlistAtomic $document $configPath;Write-DevintoshStepLog $step 'PlatformInfo/Generic is enabled, SMBIOS update policy is active, and SecureBootModel is Disabled for first boot.' 'PASS'
    $step++;Write-DevintoshProgress $step $totalSteps 'Writing first-boot configuration report';$report=[ordered]@{schemaVersion=2;generatedAtUtc=(Get-Date).ToUniversalTime().ToString('o',[Globalization.CultureInfo]::InvariantCulture);status='Applied';platformInfoGenericEnabled=$true;updateDataHub=$true;updateNVRAM=$true;updateSMBIOS=$true;updateSMBIOSMode='Create';secureBootModel='Disabled';scope='first-boot-recovery';reason='Enable the ephemeral bootstrap SMBIOS identity and avoid Secure Boot model system-id dependency before native validation.';configPath='build/efi/EFI/OC/config.plist'};$report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath $reportPath -Encoding UTF8;Write-DevintoshStepLog $step 'First-boot configuration report written.' 'PASS'
