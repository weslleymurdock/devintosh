...
.DESCRIPTION
    Downloads the already-pinned OpenCore RELEASE archive defined by the Sequoia
    configuration, verifies its SHA-256 digest, extracts Utilities/ocvalidate, and
    validates build/efi/EFI/OC/config.plist.

    No OpenCore binary is committed to the repository. Validation is tied to the same
    pinned OpenCore release used by build-opencore.ps1.

.EXIT CODES
    0 = OpenCore configuration is valid.
    1 = General failure.
    ...
    7 = OpenCore asset integrity failure.
...
try {
    $step++
    Write-DevintoshProgress $step $totalSteps 'Checking OpenCore candidate and pinned release configuration'
    ...
    $url = [string]$config.opencore.releaseUrl
    $expectedSha = ([string]$config.opencore.releaseSha256).ToLowerInvariant()
    $version = [string]$config.opencore.version
    if ([string]::IsNullOrWhiteSpace($url) -or [string]::IsNullOrWhiteSpace($expectedSha) -or [string]::IsNullOrWhiteSpace($version)) {
        $EXIT_CODE=$script:EXIT_VALIDATION_FAILURE
        throw 'Pinned OpenCore release configuration is incomplete.'
    }
    Write-DevintoshStepLog $step "OpenCore $version validation target is ready." 'PASS'

    $step++
    Write-DevintoshProgress $step $totalSteps 'Preparing isolated ocvalidate workspace'
    if (-not (Test-Path -LiteralPath $toolRoot)) { New-Item -ItemType Directory -Path $toolRoot -Force | Out-Null }
    if (Test-Path -LiteralPath $extractRoot) { Remove-Item -LiteralPath $extractRoot -Recurse -Force }
    Write-DevintoshStepLog $step 'Validation workspace prepared.' 'PASS'

    $step++
    Write-DevintoshProgress $step $totalSteps 'Downloading pinned OpenCore release for validation'
    ...
    $step++
    Write-DevintoshProgress $step $totalSteps 'Verifying pinned OpenCore release integrity'
    $actualSha = Get-FileSha256 $archivePath
    if ($actualSha -ne $expectedSha) {
        $EXIT_CODE=$script:EXIT_ASSET_INTEGRITY_FAILURE
        throw "OpenCore validation archive SHA-256 mismatch. Expected $expectedSha; received $actualSha."
    }
    Write-DevintoshStepLog $step "OpenCore release SHA-256 verified: $actualSha." 'PASS'

    $step++
    Write-DevintoshProgress $step $totalSteps 'Extracting pinned ocvalidate utility'
    Expand-Archive -LiteralPath $archivePath -DestinationPath $extractRoot -Force
    $candidate = Join-Path $extractRoot 'Utilities\ocvalidate\ocvalidate.exe'
    if (-not (Test-Path -LiteralPath $candidate)) {
        $EXIT_CODE=$script:EXIT_ASSET_INTEGRITY_FAILURE
        throw "Pinned OpenCore archive is missing Utilities/ocvalidate/ocvalidate.exe: $candidate"
    }
    ...
    $step++
    Write-DevintoshProgress $step $totalSteps 'Validating generated config.plist with ocvalidate'
    $process = Start-Process -FilePath $validatorPath -ArgumentList @($configPath) -NoNewWindow -Wait -PassThru
    $exitCode = [int]$process.ExitCode
    if ($exitCode -ne 0) {
        $EXIT_CODE=$script:EXIT_VALIDATION_FAILURE
        throw "ocvalidate rejected config.plist with exit code $exitCode."
    }
    Write-DevintoshStepLog $step 'ocvalidate accepted the generated config.plist.' 'PASS'

    $step++
    Write-DevintoshProgress $step $totalSteps 'Writing OpenCore validation report'
    $report = [ordered]@{
        schemaVersion = 1
        generatedAtUtc = (Get-Date).ToUniversalTime().ToString('o', [Globalization.CultureInfo]::InvariantCulture)
        validator = 'OpenCorePkg Utilities/ocvalidate'
        openCoreVersion = $version
        releaseSha256 = $actualSha
        configPath = 'build/efi/EFI/OC/config.plist'
        status = 'Valid'
    ...
    Write-DevintoshStepLog $step 'OpenCore validation report written.' 'PASS'
    Complete-DevintoshProgress 'OpenCore config validation complete'
    exit $script:EXIT_SUCCESS
}
catch {
    Write-DevintoshStepLog $step "OpenCore validation failed: $($_.Exception.Message)" 'FAIL'
