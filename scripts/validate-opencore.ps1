...
function Test-FirstBootPrerequisites {
    ...
    $uefi=Get-PlistDictionary $root @('UEFI');if($null -eq $uefi){throw 'UEFI dictionary is missing.'};$drivers=Get-PlistValue $uefi 'Drivers';if($null -eq $drivers -or $drivers.Name-ne 'array'){throw 'UEFI/Drivers array is missing.'};$hfsEntry=@($drivers.ChildNodes|Where-Object{$_.NodeType-eq 'Element' -and $_.Name-eq 'string' -and $_.InnerText-eq 'HfsPlus.efi'});if($hfsEntry.Count-eq 0){throw 'UEFI/Drivers does not reference HfsPlus.efi.'}
    return [ordered]@{platformInfoAutomatic=$true;systemProductNamePresent=$true;systemSerialNumberPresent=$true;mlbPresent=$true;systemUuidValid=$true;romBytes=$romBytes.Length;platformInfoUpdatesEnabled=$true;secureBootModel=$secureBootModel.InnerText;hfsPlusPresent=$true;hfsPlusPath='build/efi/EFI/OC/Drivers/HfsPlus.efi'}
}
try{
