...
<#
.SYNOPSIS
    Applies matched, validated OpenCore profile fragments to the generated candidate.

.DESCRIPTION
    Hardware-agnostic application stage. It consumes the profile resolution produced by
    configure-opencore.ps1 and applies only declarative `opencore.plist` fragments from
    matched profiles.

    Profiles marked `opencore.policy = validation-required` are never applied. Unknown
    hardware and unresolved capabilities remain non-fatal and are preserved in the report.
    Conflicting writes to the same plist path are rejected before any change is made.

.PARAMETER Force
    Replaces the current generated config.plist after creating a backup.

.EXIT CODES
    0 = Profile fragments applied successfully; unresolved capabilities may remain.
    1 = General failure.
...
try {
    ...
    $preview = Apply-OpenCoreProfileFragments -Root ([System.Xml.XmlElement]$null) -Profiles @()
    ...
    if ($application.status -eq 'Conflict') {
        $EXIT_CODE=$script:EXIT_VALIDATION_FAILURE
        throw 'OpenCore profile fragment conflict detected during application.'
    }
    ...
    Add-DevintoshRollbackAction "Restore OpenCore candidate from $backupPath" {
        if (Test-Path -LiteralPath $backupPath) { Copy-Item -LiteralPath $backupPath -Destination $configPath -Force }
    }
