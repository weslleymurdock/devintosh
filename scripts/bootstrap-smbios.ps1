...
function New-Identity {
    ...
    $rom=([BitConverter]::ToString($bytes)).Replace('-','')
...
try {
    ...
    $step++;Write-DevintoshProgress $step $totalSteps 'Finalizing SMBIOS bootstrap';Complete-DevintoshTransaction;Complete-DevintoshProgress 'SMBIOS bootstrap complete';Write-DevintoshStepLog $step 'OpenCore now has a non-zero local SystemUUID and a resolved Mac model for first-boot testing.' 'PASS'
