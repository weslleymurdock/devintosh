...
$sampleUri = 'https://raw.githubusercontent.com/{0}/{1}/Docs/Sample.plist' -f [string]$openCoreProfile.repository,[string]$openCoreProfile.tag
...
function Ensure-PlistArrayDictionaryEntryLocal {
    param([Parameter(Mandatory)][System.Xml.XmlElement]$Dict,[Parameter(Mandatory)][string]$Name,[Parameter(Mandatory)][string]$Path)
    ...
    $entries=@($array.ChildNodes|Where-Object{$_.NodeType -eq 'Element'})
    foreach($entry in $entries){
        if($entry.Name -ne 'dict'){throw "Plist array '$Name' contains an invalid entry of type '$($entry.Name)'."}
        $entryPath=$null
        $pathKey=@($entry.ChildNodes|Where-Object{$_.NodeType -eq 'Element' -and $_.Name -eq 'key' -and $_.InnerText -eq 'Path'})
        if($pathKey.Count -gt 1){throw "Duplicate Path key in plist array '$Name'."}
        if($pathKey.Count -eq 1){$pathValue=$pathKey[0].NextSibling;while($null -ne $pathValue -and $pathValue.NodeType -ne 'Element'){$pathValue=$pathValue.NextSibling};if($null -ne $pathValue -and $pathValue.Name -eq 'string'){$entryPath=$pathValue.InnerText}}
        if($entryPath -eq $Path){Set-PlistValueLocal $entry 'Enabled' 'boolean' 'true';return}
    }
    $entry=$Dict.OwnerDocument.CreateElement('dict')
    $enabledKey=$Dict.OwnerDocument.CreateElement('key');$enabledKey.InnerText='Enabled';[void]$entry.AppendChild($enabledKey);$enabled=$Dict.OwnerDocument.CreateElement('true');[void]$entry.AppendChild($enabled)
    $loadEarlyKey=$Dict.OwnerDocument.CreateElement('key');$loadEarlyKey.InnerText='LoadEarly';[void]$entry.AppendChild($loadEarlyKey);$loadEarly=$Dict.OwnerDocument.CreateElement('false');[void]$entry.AppendChild($loadEarly)
    $pathKey=$Dict.OwnerDocument.CreateElement('key');$pathKey.InnerText='Path';[void]$entry.AppendChild($pathKey);$pathValue=$Dict.OwnerDocument.CreateElement('string');$pathValue.InnerText=$Path;[void]$entry.AppendChild($pathValue)
    [void]$array.AppendChild($entry)
}
...
try {
    ...
    $step++; Write-DevintoshProgress $step $totalSteps "Loading pinned OpenCore $($openCoreProfile.version) plist schema";Invoke-WebRequest -Uri $sampleUri -UseBasicParsing -OutFile $samplePath;if(-not(Test-Path -LiteralPath $samplePath)){$EXIT_CODE=$script:EXIT_DEPENDENCY_FAILURE;throw 'OpenCore Sample.plist could not be obtained.'};$xml=New-Object System.Xml.XmlDocument;$xml.PreserveWhitespace=$true;$xml.Load($samplePath);$root=[System.Xml.XmlElement]$xml.DocumentElement.SelectSingleNode('/plist/dict');if($null -eq $root){$EXIT_CODE=$script:EXIT_ASSET_INTEGRITY_FAILURE;throw 'Invalid OpenCore Sample.plist root.'};Remove-PlistWarningsLocal $root;Write-DevintoshStepLog $step "Pinned OpenCore $($openCoreProfile.version) schema loaded for $($versionProfile.macOSFamily)." 'PASS'
    $step++; Write-DevintoshProgress $step $totalSteps 'Applying hardware-independent OpenCore safety defaults';$bootQuirks=Get-PlistDictLocal $root @('Booter','Quirks');$uefiQuirks=Get-PlistDictLocal $root @('UEFI','Quirks');$security=Get-PlistDictLocal $root @('Misc','Security');$nvram=Get-PlistDictLocal $root @('NVRAM','Add','7C436110-AB2A-4BBB-A880-FE41995C9F82');$uefi=Get-PlistDictLocal $root @('UEFI');Ensure-PlistArrayDictionaryEntryLocal $uefi 'Drivers' 'HfsPlus.efi';Set-PlistValueLocal $bootQuirks 'AvoidRuntimeDefrag' 'boolean' 'true';Set-PlistValueLocal $uefiQuirks 'RequestBootVarRouting' 'boolean' 'true';Set-PlistValueLocal $security 'AllowSetDefault' 'boolean' 'true';Set-PlistValueLocal $security 'Vault' 'string' 'Optional';Set-PlistValueLocal $nvram 'boot-args' 'string' '';Write-DevintoshStepLog $step 'Only generic schema-safe defaults were applied; UEFI driver entries preserve the schema supplied by the pinned OpenCore Sample.plist.' 'PASS'
    ...
    $step++; Write-DevintoshProgress $step $totalSteps 'Writing OpenCore configuration candidate';if(-not(Test-Path -LiteralPath $efiOcRoot)){New-Item -ItemType Directory -Path $efiOcRoot -Force|Out-Null};if((Test-Path -LiteralPath $configPath) -and -not $Force){$EXIT_CODE=$script:EXIT_VALIDATION_FAILURE;throw 'Existing config.plist found. Use -Force to replace it.'};Save-PlistUtf8Atomic -Xml $xml -Path $configPath;$resolution=[ordered]@{schemaVersion=2;generatedAtUtc=(Get-Date).ToUniversalTime().ToString('o',[Globalization.CultureInfo]::InvariantCulture);sourceHardware='build/opencore/hardware-detected.json';targetId=[string]$versionProfile.id;macOSFamily=[string]$versionProfile.macOSFamily;macOSMajorVersion=[int]$versionProfile.macOSMajorVersion;openCoreVersion=[string]$openCoreProfile.version;openCoreTag=[string]$openCoreProfile.tag;matchedProfiles=@($matched|ForEach-Object{[string](Get-Prop $_ 'id')});status=$state.status;resolvedCapabilities=@($state.resolved);unresolvedCapabilities=@($state.unresolved);needsValidation=@($state.needsValidation);capabilities=$state.capabilities};$resolution|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $resolutionPath -Encoding UTF8;$report=[ordered]@{schemaVersion=3;generatedAtUtc=(Get-Date).ToUniversalTime().ToString('o',[Globalization.CultureInfo]::InvariantCulture);sourceProfile='build/opencore/hardware-detected.json';targetId=[string]$versionProfile.id;macOSFamily=[string]$versionProfile.macOSFamily;macOSMajorVersion=[int]$versionProfile.macOSMajorVersion;openCoreVersion=[string]$openCoreProfile.version;openCoreTag=[string]$openCoreProfile.tag;status=$state.status;matchedProfiles=@($matched|ForEach-Object{[string](Get-Prop $_ 'id')});resolvedCapabilities=@($state.resolved);unresolvedCapabilities=@($state.unresolved);needsValidation=@($state.needsValidation);appliedCapabilityKeys=@();generatedArtifacts=@('build/efi/EFI/OC/config.plist');intentionallyNotGenerated=@('SMBIOS unique identifiers','audio layout-id','USB port map','ACPI patches and SSDTs','GPU spoofing','third-party kext binaries and versions','OpenCore vault files')};$report|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $reportPath -Encoding UTF8;Complete-DevintoshTransaction;Write-DevintoshStepLog $step "OpenCore configuration candidate written to $configPath." 'PASS'
    $step++; Write-DevintoshProgress $step $totalSteps 'Finalizing hardware-agnostic OpenCore configuration';Complete-DevintoshProgress 'Finalizing hardware-agnostic OpenCore configuration';Write-DevintoshStepLog $step "Target $($versionProfile.macOSFamily) is coupled to OpenCore $($openCoreProfile.version) through the declarative version profile; hardware policy remains profile-driven." 'PASS';exit $script:EXIT_SUCCESS
}
catch {Write-DevintoshStepLog $step "OpenCore configuration failed: $($_.Exception.Message)" 'FAIL';Write-DevintoshLog 'ERROR' $_.Exception.ToString();$rollbackOk=Invoke-DevintoshRollback;if(-not $rollbackOk){$EXIT_CODE=$script:EXIT_ROLLBACK_FAILURE};if($EXIT_CODE -eq $script:EXIT_SUCCESS){$EXIT_CODE=$script:EXIT_GENERAL_FAILURE};exit $EXIT_CODE}
