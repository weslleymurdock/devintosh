...
function Get-CapabilityStateLocal {
    ...
    foreach ($name in $names) {
        ...
        foreach ($profile in @(Get-ArraySafe $Profiles)) {
            $caps=Get-Prop $profile 'capabilities'; $enabled=Get-Prop $caps $name
            if ($null -ne $enabled -and [bool]$enabled) {[void]$providers.Add($profile)}
        }
